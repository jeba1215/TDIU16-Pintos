...
inode_read_at (struct inode *inode, void *buffer_, off_t size, off_t offset)
{
  ...
  lock_acquire(&inode->counter_lock);
  inode->readers++;
  if (inode->readers == 1)
  {
    sema_down(&inode->access_sema);
  }
  lock_release(&inode->counter_lock);
    ...
    off_t offset)
{
  ...
  //lock_acquire(&inode->waiting_lock);
  sema_down(&inode->access_sema);
  //lock_release(&inode->waiting_lock);
